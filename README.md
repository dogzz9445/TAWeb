...
- Inside the `backend` folder, do the following:
  - Create a copy of `TAWeb/settings/local.py.example`:  
  `cp TAWeb/settings/local.py.example TAWeb/settings/local.py`
...
- Open a new command line window and go to the project's directory.
- Create a new virtualenv with either [virtualenvwrapper](https://virtualenvwrapper.readthedocs.io/en/latest/) or only virtualenv: `mkvirtualenv TAWeb` or `python -m venv TAWeb-venv`
...
- Make sure the virtualenv is activated `workon TAWeb` or `source TAWeb-venv/bin/activate`
...
- `workon TAWeb` or `source TAWeb-venv/bin/activate` depending on if you are using virtualenvwrapper or just virtualenv.
